...
- Do not claim Novel Craft can guarantee an award, ranking, rights status, adaptation, popularity, or reader response.
...
4. `novel-craft scene create` records the target scene's goal, conflict, turn, stakes, exit state, threads, open loops, and repetition warnings.
5. `novel-craft plot thread` maintains state-machine records for mysteries, powers, obligations, rivalries, and long-running open loops.
6. `novel-craft matrix build/audit` maintains story progression and checks repetition, open-loop load, and therefore/because causality.
7. `novel-craft matrix heatmap` shows hot, stale, or unpaid open loops and threads.
...
17. `novel-craft memory commit` commits approved facts, open loops, and state changes.
18. `novel-craft full-book` audits a folder or manuscript for repeated issues, long-form continuity, and open-loop drift.
...
- A user can produce an open-loop heat map from the story matrix so the writer can escalate, pay off, reframe, or intentionally defer open loops.
...
- A user can generate a context packet that includes project config, characters, knowledge state, relationship state, scene card, recent scenes, open loops, plot threads, style profile, and memory events.
